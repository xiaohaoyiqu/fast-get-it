...
整个过程不要求在软件里填写账号或密码，也不会保存密码。Cookie 等同登录凭证，只保存在 `data/software_app/`，不要提交到 Git 或发送给他人。
...
Pixiv 和 FANBOX 必须分别选择并登录一次，因为它们使用不同域名和会话 Cookie。第二次获取会合并文件，不会擦除第一次的登录资料。E-Hentai 表站与 ExHentai 里站也必须分别选择；两个账号允许不同，两个普通 Chrome 资料和 Cookie 文件不会合并或互相复用。登录期间应用不连接浏览器；关闭窗口后才读取 Chrome Cookie 数据库并在线验证。里站登录和后续访问应保持同一代理/VPN 出口。
...
先点击设置页底部的 **检测浏览器驱动**；缺少时点 **安装缺失驱动**。还应确认 Google Chrome 可用，并且没有安全软件阻止临时浏览器资料目录。
